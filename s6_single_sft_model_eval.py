...
def init_model(lm_config):
    ...
    model_from = 1  # 1从本地，2用transformers

    if model_from == 1:
        moe_path = '_moe' if lm_config.use_moe else '' # 是否使用专家模型
        ckp = f'./out/single_sft_{lm_config.dim}{moe_path}.pth' # 预训练模型参数路径

        model = Transformer(lm_config)# 模型初始化
        state_dict = torch.load(ckp, map_location=device)#加载参数
    ...
    while True:
        ...
        with torch.no_grad():
            res_y = model.generate(x, tokenizer.eos_token_id, max_new_tokens=max_seq_len, temperature=temperature,
                                   top_k=top_k, stream=stream,repetition_penalty=repetition_penalty)
            print('回答：', end='')
            try:
                y = next(res_y)
