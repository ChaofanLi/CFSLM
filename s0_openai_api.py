...
MODEL_PATH = "./cfslm-v1-small-multi-sft"
...
class CreateChatCompletionSchema(Schema):
    ...
    model = fields.Str(required=True, metadata={"example": "cfslm"})
...
class ChatCompletionSchema(Schema):
    ...
    created = fields.Int(dump_default=lambda: int(time.time()), metadata={"example": 1695402567})
    model = fields.Str(metadata={"example": "cfslm"})
...
if __name__ == '__main__':
    ...
    embeddings_model_m3e = SentenceTransformer('./embedding_models/all-MiniLM-L6-v2', device='cpu') if use_emb else None
    embeddings_model_bge = SentenceTransformer('./embedding_models/all-MiniLM-L6-v2', device='cpu') if use_emb else None
